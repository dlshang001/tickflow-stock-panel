...
async function* getStream(tab: ReviewTab, focus: string): AsyncGenerator<StreamEvent> {
  switch (tab) {
    case 'market':
      yield* api.reviewStream(undefined, focus)
      break
    case 'holdings':
      yield* api.positionAnalyzeStream(focus)
      break
    case 'settlement':
      yield* api.settlementAnalyzeStream(focus)
      break
  }
}
 ...
 * @param onDone 完成回调(供 market tab 自动归档;holdings/settlement 由后端自动归档)
 */
export async function startGeneration(
  ...
  try {
    for await (const evt of getStream(tab, focus)) {
